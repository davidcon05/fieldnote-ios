...
**Goal:** Enable users to search across ALL journals from the Dashboard, not just the current journal.

**Current State (v1.0):**
- ✅ Dashboard search: Search journal names only
- ✅ Logs List search: Search notes/transcriptions within current journal
- ❌ No way to search ALL logs across ALL journals

**Proposed Enhancement:**

#### Option A: Dashboard Global Search (Recommended)
When user types in Dashboard search bar, search across:
1. **Journal names** (current behavior)
2. **All log notes** (NEW)
3. **All audio transcriptions** (NEW)
4. **Tags** (future feature)
...
│   • Sparrow nest found - May 18, 2026   │
│                                          │
│ Backyard Journal                         │
│   • Sparrow at feeder - May 20, 2026    │
│   • Sparrow behavior notes - May 22     │
└─────────────────────────────────────────┘
```
...
var globalSearchResults: [Log] {
    ...
    let allLogs = journals.flatMap { $0.logs }
...
var searchResultsByJournal: [Journal: [Log]] {
    Dictionary(grouping: globalSearchResults) { log in
        log.journal!
    }
}
```
...
**UI Changes:**
- Dashboard search now has **2 sections**:
  1. **Journals** (existing behavior)
  2. **Logs** (NEW - shows log results grouped by journal)
- Tap log result → Navigate directly to that log's detail view
- Show match count: "5 logs found in 2 journals"

**Benefits:**
- ✅ Find forgotten notes from months ago
- ✅ Cross-journal discovery (e.g., "Where did I see that sparrow?")
- ✅ Leverages existing search infrastructure
- ✅ Useful for both research (find all observations of X) and personal (find that trip to Paris)

---

#### Option B: Dedicated "Search All" Tab (Alternative)
Add new tab in bottom navigation: **Search** tab

- Global search bar at top
- Search across all journals + logs
- Filter results by journal, date, location
- Similar to iOS Spotlight search

**Pros:**
- ✅ Dedicated space for powerful search
- ✅ Room for advanced filters (date range, location radius, tags)

**Cons:**
- ❌ More complex navigation
- ❌ Adds another tab (bottom nav gets crowded)
- ❌ Duplicates search functionality across 3 places

**Recommendation:** Start with **Option A** (Dashboard Global Search). If users demand more advanced search, add dedicated Search tab in v1.2+.

---

#### Implementation Scope

**Phase 1: Basic Global Search (v1.1)**
- [ ] Search log titles across all journals
- [ ] Search log notes across all journals
- [ ] Search audio transcriptions across all journals
- [ ] Group results by journal
- [ ] Tap result → Navigate to log detail

**Phase 2: Advanced Search (v1.2+)**
- [ ] Filter by date range (this week, this month, custom)
- [ ] Filter by journal (select which journals to search)
- [ ] Filter by content type (notes only, audio only, photos only)
- [ ] Search within GPS radius (find logs near location)
- [ ] Search by tags (future tagging feature)
- [ ] Sort results (relevance, date, location)

**Phase 3: Search Analytics (v1.3+)**
- [ ] "Recently searched" suggestions
- [ ] Most common searches
- [ ] Auto-complete suggestions based on log content

**Estimated Time:**
- Phase 1: 2-3 days (leverages existing search code)
- Phase 2: 1 week (advanced filters)
- Phase 3: 3-5 days (analytics + suggestions)

**Milestone:** v1.1 (Phase 1), v1.2+ (Phase 2-3)

---

### Photo Metadata Extraction & Auto-Tagging
**Status:** Not implemented
**Priority:** Medium-High (v1.1 or v1.2)

**Goal:** Extract GPS, date/time, and other metadata from photos to auto-populate log fields and enable smart tagging (e.g., bird species tracking).

**Use Cases:**

#### Use Case 1: "Birds Journal" - Species Tracking
User takes photo of bird → App extracts metadata → User tags species → App tracks:
- GPS location of sighting
- Date/time from photo EXIF
- Camera direction (compass bearing)
- Weather at that location/time
- Automatic timeline of species sightings

#### Use Case 2: Import Photos with Existing GPS
User selects photo from library (taken earlier) → App extracts GPS → Auto-populates log location (instead of using current device location)

#### Use Case 3: Photography Journal
...
#### Use Case 4: Travel Journal
Auto-detect location from photo → Suggest log title based on location name (reverse geocoding)

---

**What Metadata Can We Extract?**

iOS provides rich EXIF metadata via `ImageIO` framework:
...
class PhotoMetadataService {
    func extractMetadata(from image: UIImage) -> PhotoMetadata? {
        ...
        return PhotoMetadata(
            gps: extractGPS(from: metadata),
            dateTime: extractDateTime(from: metadata),
            camera: extractCameraInfo(from: metadata),
            orientation: metadata["Orientation"] as? Int
        )
    }

    func extractGPS(from metadata: [String: Any]) -> GPSData? {
        guard let gps = metadata["{GPS}"] as? [String: Any],
        ...
        return GPSData(
            latitude: lat,
            longitude: lon,
            altitude: gps["Altitude"] as? Double,
            direction: gps["ImgDirection"] as? Double, // Compass direction camera was facing
            speed: gps["Speed"] as? Double // Speed when photo taken
        )
    }

    func extractDateTime(from metadata: [String: Any]) -> Date? {
        guard let dateString = metadata["DateTimeOriginal"] as? String else {
            return nil
        }
        // Parse "2026:05:25 14:32:18" format
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy:MM:dd HH:mm:ss"
        return formatter.date(from: dateString)
    }

    func extractCameraInfo(from metadata: [String: Any]) -> CameraInfo? {
        return CameraInfo(
            make: metadata["Make"] as? String,
            model: metadata["Model"] as? String,
            lens: metadata["LensModel"] as? String,
            fNumber: metadata["FNumber"] as? Double,
            exposureTime: metadata["ExposureTime"] as? String,
            iso: metadata["ISOSpeedRatings"] as? Int,
...
struct GPSData {
    ...
    let altitude: Double?
    let direction: Double? // 0-360 degrees (0 = North, 90 = East, etc.)
    let speed: Double? // Speed in m/s when photo taken
}

struct CameraInfo {
    let make: String? // "Apple"
    let model: String? // "iPhone 14 Pro"
    let lens: String? // "iPhone 14 Pro back triple camera 6.86mm f/1.78"
    let fNumber: Double? // 1.78
    let exposureTime: String? // "1/120"
    let iso: Int? // 64
    let focalLength: Double? // 6.86mm
}
```
...
#### Phase 1: Auto-Populate from Photo Metadata (v1.1)

**When user adds photo to log:**
1. Extract metadata from photo
2. If photo has GPS BUT log doesn't → Use photo's GPS for log
3. If photo has DateTime BUT log timestamp is "now" → Suggest using photo's timestamp
4. Show notification: "📸 Photo has GPS data. Use for this log?"

**UI:**
```swift
// When photo added to NewLogView or EditLogView:
if let photoGPS = photoMetadata.gps, log.latitude == nil {
    // Show alert
    Alert(
        title: "Use Photo Location?",
        message: "This photo was taken at \(photoGPS.latitude), \(photoGPS.longitude). Use this location for the log?",
        primaryButton: .default(Text("Use Photo GPS")) {
            log.latitude = photoGPS.latitude
            log.longitude = photoGPS.longitude
            log.altitude = photoGPS.altitude
        },
        secondaryButton: .cancel(Text("Use Current Location"))
    )
}
```

**Benefits:**
- ✅ Import old photos with historical GPS
- ✅ Create logs for past events (vacations, hikes)
- ✅ No need to be at location to create log

---

#### Phase 2: Species Tagging System (v1.2)
...
class Log {
    // ... existing fields
    var tags: [String] = [] // NEW: Array of tags (e.g., ["Northern Flicker", "Woodpecker", "Bird"])
}
```

**UI: Tag Input After Photo**
```
User adds bird photo →
┌─────────────────────────────────────────┐
│ 🐦 Add Species Tag                      │
├─────────────────────────────────────────┤
│ Species Name:                            │
...
**Search by tag:**
- Dashboard global search: Include tags in search
- Filter logs by tag: "Show all 'Northern Flicker' sightings"
...
- ✅ Share species lists with other researchers

---

#### Phase 3: AI Species Identification (v2.0+ - Optional)

Use CoreML or API to auto-identify species:

**Option A: iNaturalist Computer Vision API**
- Free, 70k+ species
- RESTful API
- Confidence scores
- Community-verified results

**Option B: Cornell Lab's Merlin Bird ID API**
- Bird-specific (10k+ species)
- Highly accurate
- Audio identification too (future feature)
...
1. Upload photo to iNaturalist API (or run CoreML model)
2. Get top 3 suggestions with confidence scores
3. Show suggestions to user: "Is this a Northern Flicker? (87% confidence)"
4. User confirms or selects different suggestion
5. Tag saved to log
```

**Benefits:**
- ✅ Automatic identification (less user effort)
- ✅ Educational (learn species names)
- ✅ Fun UX (AI feels magical)

**Cons:**
- ❌ Complex implementation
- ❌ Requires internet (unless CoreML)
- ❌ May misidentify (requires user verification)

**Recommendation:** Start with **Phase 1-2** (manual tagging). If users love it, add AI in v2.0+.

---

**Implementation Scope:**

**Phase 1: Metadata Extraction (v1.1)**
- [ ] PhotoMetadataService implementation
- [ ] Extract GPS from photos
- [ ] Extract DateTime from photos
- [ ] Auto-populate log fields from photo metadata
- [ ] User confirmation prompts ("Use photo GPS?")

**Phase 2: Manual Tagging (v1.2)**
- [ ] Add `tags: [String]` to Log model
- [ ] Tag input UI (after photo capture)
- [ ] Tag search (global search includes tags)
- [ ] Tag filter (show logs with specific tag)
- [ ] Tag statistics (count species, most common tags)
- [ ] Tag autocomplete (suggest previously used tags)

**Phase 3: Camera Metadata Display (v1.2)**
- [ ] Extract camera settings (f-stop, ISO, shutter speed)
- [ ] Display in log detail view (expandable "Camera Info" section)
- [ ] Photography journal use case (track settings that work)

**Phase 4: AI Species ID (v2.0+)**
- [ ] Research APIs (iNaturalist, Merlin, CoreML options)
- [ ] Integrate API or train model
- [ ] Species suggestion UI
- [ ] User verification workflow
- [ ] Confidence score display

**Estimated Time:**
- Phase 1: 3-4 days (metadata extraction + UI)
- Phase 2: 5-7 days (tagging system + search integration)
- Phase 3: 2-3 days (camera metadata display)
- Phase 4: 2-3 weeks (AI integration + testing)

**Milestone:** v1.1 (Phase 1-2), v1.2 (Phase 3), v2.0+ (Phase 4)
